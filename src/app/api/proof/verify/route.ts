...
import { getAuthAgent, requireAuth } from "@/lib/auth";
import { computeDailyNonce, bufferToHex } from "@/lib/proof";
import { eq, sql } from "drizzle-orm";

export async function POST(req: NextRequest) {
  // Require JWT auth
  const authAgent = requireAuth(await getAuthAgent(req));
  ...
  if (hash !== expectedHash) {
    return NextResponse.json(
      { error: "Hash mismatch. Expected SHA-256(your_public_key + \":\" + nonce)" },
      { status: 400 }
    );
  }
